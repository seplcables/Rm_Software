...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
        ...
        $this->Cell(0, 8, '**This Is Computer Generated PO Signature Is Not Required**', 0, false, 'C', 0, '', 0, false, 'T', 'M');
        // Page number
...
// Logo
		...
		$image_file = '1.jpeg';
        $pdf->Image($image_file, 5, 5, 20, 20, '', '', 'T', false, 300, '', false, false, 0, false, false, false);
        // Set font
        ...
        $pdf->SetX(230);
        $pdf->Cell(0, 8, 'Po. No.: - '.$row['po_no']);
        $pdf->Ln(7);
		$pdf->SetX(35);
		$pdf->SetFont("helvetica", "I", 10);
		$pdf->Cell(0, 20, "GSTIN: 24AACCS9412R1ZV");
		$pdf->SetX(230);
		$pdf->Cell(0, 8, 'Date: - '.$row['po_date']->format('d-M-Y'));
		$pdf->Ln(7);
		$pdf->SetX(230);
		$pdf->Cell(0, 8, 'Last Delivery Date:-'.$row['mat_req_date']->format('d-M-Y'));
		$pdf->Ln(1);
		$pdf->SetX(2);
		$pdf->Cell(0, 10, '___________________________________________________________________________________________________________________________________________________');				
		...
		$output='<table border="0.2" cellspacing="0">';
			$output .= '<tr bgcolor="#cccccc">';
				...
				$output .= '<th width="105" align="center">Item_Description</th>';
				$output .= '<th width="105" align="center">Make_by</th>';
				$output .= '<th width="105" align="center">Model_no</th>';
				...
				$output .= '<th width="65" align="center">Basic_Val</th>';
			$output .= '</tr>';
			...
			$output .= '<tr>';
				...
				$output .= '<td width="105" align="center">'.$row1["item"].'</td>';
				$output .= '<td width="105" align="center">'.$row["make_by"].'</td>';
				$output .= '<td width="105" align="center">'.$row["model_no"].'</td>';
				...
				$output .= '<td width="65" align="center">'.$row["basic_rate"].'</td>';
			$output .= '</tr>';

    }

    $output .= '<tr color="#0000ff">';
    $output .= '<td colspan="9" align="right"><b>TOTAL =>___ </b></td>';
...
$pdf->Ln(14);
	...
	while ($row3 = sqlsrv_fetch_array($run3, SQLSRV_FETCH_ASSOC)) {
			...
			$output1 .= '<tr color="#000000">';
				...
				$output1 .= '<td width="205" align="center"><b><i>'.$row3["title"].'</b></i></td>';
				$output1 .= '<td width="560" align="center">'.$row3["descriptions"].'</td>';
			$output1 .= '</tr>';
...
$pdf->Ln(14);
	...
	while ($row4 = sqlsrv_fetch_array($run4, SQLSRV_FETCH_ASSOC)) {
			...
			$output2 .= '<tr color="#000000">';
				...
				$output2 .= '<td width="205" align="center"><b><i>'.$row4["location"].'</b></i></td>';
				$output2 .= '<td width="560" align="center">'.$row4["location_address"].'</td>';
			$output2 .= '</tr>';
