...
$sql = "SELECT distinct a.id, format(a.po_date,'dd-MMM-yyyy') as poDate, b.party_name,a.po_gen_by,format(a.mat_req_date,'dd-MMM-yyyy') as reqDate,DATEDIFF(day, a.mat_req_date,GETDATE()) AS DateDiff from po_entry_head a left outer join rm_party_master b on b.pid = a.party where  a.po_date > '2021-06-01' and DATEDIFF(day, a.mat_req_date,GETDATE()) > 0 order by a.id desc";
...
while($row = sqlsrv_fetch_array($run, SQLSRV_FETCH_ASSOC))
{
    ...
    // $sql1 = "SELECT COUNT(a.item_code) as count from po_entry_details a where a.isCancle IS NULL and a.id = '".$row['poid']."'";

    // //$sql1 = "SELECT COUNT(item_code) as count from po_entry_details where iid = '".$row['id']."' and isCancle is NULL";
    // $run1 = sqlsrv_query($con,$sql1);
    // $row1 = sqlsrv_fetch_array($run1, SQLSRV_FETCH_ASSOC);
    // if($row1['count'] == 0) 
    // {
        ...
        buttons: [
            'pageLength', 'excel',
                ...
                },
                {
                    ...
                    action: function () { 
                        window.open("po_to_inward.php","_self")
                    }
                },
